(ns tanker.operation
  "OperationActor -- one marine-tanker operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (TankerAdvisor) is sealed into a single node (:advise); its proposal
  is ALWAYS routed through the Marine Cargo Governor (:govern) and the
  ...
  One graph run = one tanker operation (intake -> advise -> govern ->
  decide -> commit | hold | approval). No unbounded inner loop -- each
  operation is auditable and checkpointed. A vessel-shipment's life is
  advanced by MANY ops (intake / verify / dispatch / settle), each its
  own independent run.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human operator (tanker operator / vessel master). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:voyage/dispatch`/`:discharge/settle` ALWAYS reach this
  node when the governor is clean -- see `tanker.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [tanker.tankeradvisor :as tankeradvisor]
...
(defn build
  ...
  `tanker.store/Store`).
  opts:
    :advisor      -- a `tanker.tankeradvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (tankeradvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; TankerAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (tankeradvisor/-advise advisor store request)]
            {:proposal p :audit [(tankeradvisor/trace request p)]})))

      ;; Marine Cargo Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
