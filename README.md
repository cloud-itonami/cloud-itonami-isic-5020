...
-- see `tanker.phase`'s docstring and `test/tanker/phase_test.cljk`'s
...
```bash
kbb -M:dev:run     # walk one clean dispatch + discharge lifecycle, plus eight HARD-hold cases, through the actor
...
| File | Role |
|---|---|
| `src/tanker/store.cljk` | **Store** protocol -- `MemStore` ‖ `DatomicStore` (`langchain.db`) + append-only audit ledger + dispatch AND discharge history (dual history). The double-actuation guard checks dedicated `:dispatched?`/`:discharged?` booleans rather than a `:status` value |
| `src/tanker/registry.cljk` | Dispatch/settlement draft records, plus the self-contained tanker-safety pure functions (`imo-number-valid?` -- the SOLAS A.600(15) 7-digit check-digit scheme; `inert-gas-o2-excessive?` -- the SOLAS II-2/4.5.5 8 vol% O2 ceiling) the governor re-verifies against -- no external capability library to delegate to |
| `src/tanker/facts.cljk` | Per-jurisdiction tanker / inert-gas / ship-shore catalog with an official spec-basis citation per entry, honest coverage reporting |
| `src/tanker/tankeradvisor.cljk` | **TankerAdvisor** -- `mock-advisor` ‖ `llm-advisor`; intake / bill-of-lading verify / dispatch / settlement proposals |
| `src/tanker/governor.cljk` | **Marine Cargo Governor** -- 8 HARD checks (spec-basis · evidence-incomplete · imo-number-invalid, the freight tracking-validity discipline on the SOLAS check digit · bl-unverified · cargo-grade-mismatch · vessel-overload, the fabrication ratio discipline · inert-gas-o2-excessive, HSE-CRITICAL at BOTH dispatch AND discharge · bonding-grounding-unconfirmed) + 2 double-actuation guards + 1 soft (confidence/actuation gate) |
| `src/tanker/phase.cljk` | **Phase 0→3** -- read-only → assisted intake → assisted verify → supervised (dispatch/settlement always human; vessel-shipment intake is the ONLY auto-eligible op, no direct capital risk) |
| `src/tanker/operation.cljk` | **OperationActor** -- langgraph StateGraph |
| `src/tanker/sim.cljk` | demo driver |
